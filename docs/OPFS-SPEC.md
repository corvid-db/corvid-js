...
| B8 | A default-mode sync handle takes an **exclusive lock** on the file: further `createSyncAccessHandle`s *and* `createWritable()` streams reject with `NoModificationAllowedError` until it closes (WebKit diverges: the same contention rejects with `InvalidStateError` instead — observed Playwright WebKit 26.5; the worker absorbs the divergence, mapping in §6). OPFS storage is origin-scoped and shared across same-origin tabs (WHATWG), so the lock is effectively cross-tab. (A `mode` option now exists — read-only/readwrite-unsafe allow sharing; we use default `readwrite` exclusively.) | MDN createSyncAccessHandle + WHATWG |

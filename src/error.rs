...
//!
//! The wire form across the wasm boundary is a JSON object in the
//! thrown Error's message (`{"corvidCode":N,"corvidMessage":"..."}`) —
//! the same contract as the corvid-node napi layer. wasm-bindgen
//! cannot attach properties to thrown errors, which is why the code
//! rides in the message; `index.js` parses it and rethrows a real
//! `CorvidError` subclass of `Error`.
...
impl CorvidErr {
    ...
    /// Render the thrown JS error message (the JS layer's wire form).
    fn wire(&self) -> String {
        format!(
...
/// Minimal JSON string escaping (no serde dependency — keeps the wasm
/// graph lean for the size budget).
...
impl From<CorvidErr> for JsValue {
    fn from(e: CorvidErr) -> JsValue {
        js_sys::Error::new(&e.wire()).into()
    }
}
